...
RUN go work init ./common ./authMicro
RUN go work sync
WORKDIR /workspace/authMicro
RUN --mount=type=cache,target=/go/pkg/mod go mod download
RUN go install github.com/go-delve/delve/cmd/dlv@latest
COPY common/ /workspace/common/
COPY authMicro/ ./
RUN --mount=type=cache,target=/root/.cache/go-build \
    CGO_ENABLED=0 GOOS=linux go build -gcflags "all=-N -l" -o auth-app ./cmd/app

FROM alpine:3.19 AS run-debug
WORKDIR /app
COPY --from=builder-debug /workspace/authMicro/auth-app .
